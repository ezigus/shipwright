...
A small Express REST API with **intentional bugs** — designed as a test target for the [cct pipeline](../scripts/cct-pipeline.sh).
...
This project ships with known issues so the cct pipeline can discover, fix, and validate them automatically.
...
When using this demo with `cct pipeline`, consider creating these issues:
...
## Running with cct pipeline

```bash
# From the repo root:
cct pipeline --dir demo --issue 1 --base main
```
