    ...
    if [[ -n "${ARTIFACTS_DIR:-}" ]]; then
        ...
        jq -n \
            ...
            '{status: $status, issue_number: $issue, goal: $goal,
              task_type: $task_type, duration_seconds: $duration_s}' \
            > "$_outcome_file" 2>/dev/null || true
        if ruflo_available && declare -f ruflo_learn_from_shipwright >/dev/null 2>&1; then
        ...
        if [[ "$total_errors" -ge "$error_threshold" ]]; then
            ...
            if [[ -n "${ARTIFACTS_DIR:-}" ]]; then
                ...
                jq -n \
                    ...
                    '{status: $status, issue_number: $issue, goal: $goal,
                      task_type: $task_type, total_errors: $total_errors,
                      error_threshold: $error_threshold}' \
