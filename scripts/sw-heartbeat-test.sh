...
test_loop_has_heartbeat_checkpoint() {
    ...
    if ! grep -q "sw-heartbeat.sh" "$loop" 2>/dev/null; then
        echo -e "    ${RED}✗${RESET} sw-heartbeat.sh not referenced in loop"
        return 1
    fi
    if ! grep -q "sw-checkpoint.sh" "$loop" 2>/dev/null; then
        echo -e "    ${RED}✗${RESET} sw-checkpoint.sh not referenced in loop"
        return 1
    fi
    return 0
}
