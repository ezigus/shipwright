...
# Called by cct-daemon.sh at spawn, stage transition, completion, and failure.
...
linear_notify() {
    ...
    if [[ -f "$SCRIPT_DIR/cct-tracker.sh" ]]; then
        "$SCRIPT_DIR/cct-tracker.sh" notify "$event" "$gh_issue" "$detail" 2>/dev/null || true
        return 0
    fi
