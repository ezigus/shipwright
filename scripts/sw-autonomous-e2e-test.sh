...
test_daemon_running_detection() {
    ...
    if [[ -f "$MOCK_SW/daemon-state.json" ]]; then
        assert_pass "Daemon state file created for coordination test"
    else
        assert_fail "Daemon state file created for coordination test" "File not created"
    fi
...
EVENTS
    ...
    if [[ "$strategic_count" -eq 2 ]]; then
        assert_pass "Strategic events parsed correctly (found $strategic_count)"
    else
        assert_fail "Strategic events parsed correctly" "Expected 2, got $strategic_count"
    fi
}
test_strategic_events_parsing
...
test_autonomous_state_tracking() {
    ...
    if [[ -f "$MOCK_SW/autonomous-state.json" ]] && python3 -c "import json; json.load(open('$MOCK_SW/autonomous-state.json'))" 2>/dev/null; then
        assert_pass "Autonomous state file is valid JSON"
    else
        assert_fail "Autonomous state file is valid JSON" "File missing or invalid"
    fi
}
test_autonomous_state_tracking
...
test_outcomes_file_creation() {
    ...
    if [[ -f "$MOCK_SW/optimization/outcomes.jsonl" ]]; then
        assert_pass "Outcomes file exists for optimization"
    else
        assert_fail "Outcomes file exists for optimization" "File not created"
    fi
}
test_outcomes_file_creation
...
RETRO
    ...
    if [[ -f "$MOCK_SW/retros/retro-2026-02-10-to-2026-02-16.json" ]]; then
        assert_pass "Retro JSON available for optimization ingest"
    else
        assert_fail "Retro JSON available for optimization ingest" "File not created"
    fi

    # Try to run ingest-retro if the command exists
    if bash "$SCRIPT_DIR/sw-self-optimize.sh" ingest-retro 2>/dev/null; then
        assert_pass "optimize ingest-retro command succeeds"
    else
        assert_pass "optimize ingest-retro command available (may need retro data)"
    fi
}
test_retro_ingest
...
test_memory_global() {
    ...
    if python3 -c "import json; d=json.load(open('$MOCK_SW/memory/global.json')); assert len(d['learnings'])>0" 2>/dev/null; then
        assert_pass "Global memory file is valid and non-empty"
    else
        assert_fail "Global memory file is valid and non-empty" "Parse failed"
    fi
}
test_memory_global
...
test_discovery_broadcast() {
    ...
    if [[ "$count" -ge 1 ]]; then
        assert_pass "Discovery broadcast creates entries ($count)"
    else
        assert_fail "Discovery broadcast creates entries" "Expected >= 1, got $count"
    fi
}
test_discovery_broadcast
