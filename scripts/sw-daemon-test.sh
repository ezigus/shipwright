...
setup_env() {
    TEST_TEST_TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sw-daemon-test.XXXXXX")

    # Create events directory (daemon uses $HOME/.shipwright)
    mkdir -p "$TEST_TEST_TEMP_DIR/.shipwright"
    mkdir -p "$TEST_TEST_TEMP_DIR/logs"
    mkdir -p "$TEST_TEST_TEMP_DIR/project/.claude"

    # Set env vars to redirect daemon state
    export HOME="$TEST_TEST_TEMP_DIR"
    export EVENTS_FILE="$TEST_TEST_TEMP_DIR/.shipwright/events.jsonl"
    export DAEMON_DIR="$TEST_TEST_TEMP_DIR/.shipwright"
    export STATE_FILE="$TEST_TEST_TEMP_DIR/.shipwright/daemon-state.json"
    export LOG_FILE="$TEST_TEST_TEMP_DIR/.shipwright/daemon.log"
    export LOG_DIR="$TEST_TEST_TEMP_DIR/logs"
    export WORKTREE_DIR="$TEST_TEST_TEMP_DIR/project/.worktrees"
    export PID_FILE="$TEST_TEST_TEMP_DIR/shipwright/daemon.pid"
    export SHUTDOWN_FLAG="$TEST_TEST_TEMP_DIR/shipwright/daemon.shutdown"
...
cleanup_env() {
    if [[ -n "$TEST_TEST_TEMP_DIR" && -d "$TEST_TEST_TEMP_DIR" ]]; then
        rm -rf "$TEST_TEST_TEMP_DIR"
    fi
}
...
source_daemon_functions() {
    ...
    PROGRESS_DIR="$TEST_TEST_TEMP_DIR/progress"
...
test_metrics_json_output() {
    ...
    local output
    output=$(cd "$TEST_TEST_TEMP_DIR/project" && bash "$DAEMON_SCRIPT" metrics --json --period 1 2>&1) || true
...
test_patrol_untested_detection() {
    ...
    local mock_scripts="$TEST_TEST_TEMP_DIR/scripts"
...
test_adaptive_cycles_convergence() {
    ...
    local fns_script="$TEST_TEST_TEMP_DIR/adaptive-cycles-fns.sh"
    cat > "$fns_script" <<'FEOF'
#!/usr/bin/env bash
set -uo pipefail
emit_event() { true; }
...
test_adaptive_cycles_divergence() {
    # Create a minimal script with the pipeline_adaptive_cycles function
    local fns_script="$TEST_TEST_TEMP_DIR/adaptive-cycles-div-fns.sh"
    cat > "$fns_script" <<'FEOF'
#!/usr/bin/env bash
set -uo pipefail
emit_event() { true; }
...
test_adaptive_cycles_hard_ceiling() {
    # Create a minimal script with the pipeline_adaptive_cycles function
    local fns_script="$TEST_TEST_TEMP_DIR/adaptive-cycles-ceil-fns.sh"
    cat > "$fns_script" <<'FEOF'
#!/usr/bin/env bash
set -uo pipefail
emit_event() { true; }
...
test_adaptive_cycles_first_cycle() {
    # Create a minimal script with the pipeline_adaptive_cycles function
    local fns_script="$TEST_TEST_TEMP_DIR/adaptive-cycles-first-fns.sh"
    cat > "$fns_script" <<'FEOF'
#!/usr/bin/env bash
set -uo pipefail
emit_event() { true; }
...
test_checkpoint_expire() {
    local cp_dir="$TEST_TEST_TEMP_DIR/project/.claude/pipeline-artifacts/checkpoints"
    mkdir -p "$cp_dir"
...
EOF
    ...
    (cd "$TEST_TEST_TEMP_DIR/project" && bash "$checkpoint_script" expire --hours 1 2>/dev/null) || true
...
test_daemon_failure_closes_draft_pr() {
    ...
    if grep -q 'gh pr close.*draft_pr.*delete-branch' "$daemon_src" $DAEMON_LIB_GLOB; then
        PASS=$((PASS + 1))
        return 0
    fi
...
test_daemon_sources_vitals() {
    ...
    if grep -q 'source.*sw-pipeline-vitals.sh' "$daemon_src" $DAEMON_LIB_GLOB; then
        PASS=$((PASS + 1))
        return 0
    fi
...
test_vitals_verdict_mapping() {
    ...
    local found=0
    if grep -q 'Map vitals verdict to daemon verdict' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
...
test_vitals_event_emission() {
    ...
    if grep -q 'emit_event "pipeline.vitals_check"' "$daemon_src" $DAEMON_LIB_GLOB; then
        PASS=$((PASS + 1))
        return 0
    fi
...
test_autoscale_vitals_factor() {
    ...
    if grep -q 'max_by_vitals' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
    # Check that vitals factor caps workers when health is low
    if grep -q '_avg_health.*-lt 50' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
    # Check that vitals factor is included in min computation
    if grep -q 'max_by_vitals.*-lt.*computed' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
...
test_quality_memory_template() {
    ...
    if grep -q 'quality-scores.jsonl' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
    # Check for enterprise escalation on critical findings
    if grep -q 'critical findings.*enterprise' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
    # Check for full template on poor quality
    if grep -q 'avg.*score.*full template' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
    # Check for fast template on excellent quality
    if grep -q 'eligible for fast' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
...
test_vitals_progress_fallback() {
    ...
    if grep -q 'Vitals-based verdict.*preferred over static' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
    # Static thresholds still exist as fallback
    if grep -q 'PROGRESS_CHECKS_BEFORE_WARN' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
    if grep -q 'PROGRESS_CHECKS_BEFORE_KILL' "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
...
test_memory_query_fix() {
    local mem_dir="$TEST_TEST_TEMP_DIR/.shipwright/memory"
...
test_dora_template_escalation() {
    ...
    if grep -q "cfr.*enterprise\|enterprise.*cfr\|CFR.*enterprise" "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
    # Check for fast template eligibility
    if grep -q "eligible for fast\|fast.*eligible\|cfr.*fast" "$daemon_src" $DAEMON_LIB_GLOB; then
        found=$((found + 1))
    fi
...
test_auto_enable_self_optimize() {
    ...
    grep -q 'Auto-enabling self_optimize' "$daemon_src" $DAEMON_LIB_GLOB || \
    ...
    grep -q 'AUTO_TEMPLATE.*true.*SELF_OPTIMIZE.*false\|auto_template.*self_optimize' "$daemon_src" $DAEMON_LIB_GLOB || \
...
main() {
    ...
    setup_env
    source_daemon_functions
    echo -e "${GREEN}✓${RESET} Environment ready: ${DIM}$TEST_TEST_TEMP_DIR${RESET}"
    echo ""
