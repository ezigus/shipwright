...
remote_add() {
    ...
    local check_cmd="test -f '${sw_path}/scripts/cct' && echo 'found' || echo 'missing'"
    local result
...
remote_remove() {
    ...
    if [[ "$OPT_STOP_DAEMON" == true ]]; then
        ...
        local stop_cmd="cd '${sw_path}' && ./scripts/cct daemon stop 2>/dev/null || true"
...
remote_status() {
    ...
    for i in $(seq 0 $((count - 1))); do
        ...
        health_cmd=$(cat <<'HEALTHEOF'
            ...
            if [ -f "$HOME/.claude-teams/daemon.pid" ]; then
                daemon_pid=$(cat "$HOME/.claude-teams/daemon.pid" 2>/dev/null || true)
            ...
            if [ -d "$HOME/.claude-teams/heartbeats" ]; then
                hb_count=$(ls -1 "$HOME/.claude-teams/heartbeats/" 2>/dev/null | wc -l | tr -d ' ')
            fi
            # Count active jobs from daemon state
            if [ -f "$HOME/.claude-teams/daemon-state.json" ]; then
                active=$(python3 -c "import json; d=json.load(open('$HOME/.claude-teams/daemon-state.json')); print(len(d.get('active_jobs',{})))" 2>/dev/null || echo 0)
            fi
...
remote_deploy() {
    ...
    local verify_cmd="test -x '${sw_path}/scripts/cct' && '${sw_path}/scripts/cct' --version 2>/dev/null || echo 'verify-failed'"
    local verify_result
    # shellcheck disable=SC2086
