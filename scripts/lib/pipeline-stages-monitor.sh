...
_Generated by \`shipwright pipeline\` at $(now_iso)_"
    ...
    # Learning feedback: index pipeline outcome into ruflo HNSW for future recall
    if [[ -n "${ARTIFACTS_DIR:-}" ]]; then
        local _duration_s=0
        if [[ -n "${PIPELINE_START_EPOCH:-}" ]]; then
            _duration_s=$(( $(now_epoch) - PIPELINE_START_EPOCH ))
        fi
        local _outcome_file="$ARTIFACTS_DIR/pipeline-outcome-validate.json"
        jq -n \
            --arg status "success" \
            --arg issue "${ISSUE_NUMBER:-}" \
            --arg goal "${GOAL:-}" \
            --arg task_type "${INTELLIGENCE_ISSUE_TYPE:-unknown}" \
            --arg template "${PIPELINE_NAME:-standard}" \
            --argjson duration_s "$_duration_s" \
            --argjson stages_passed "${PIPELINE_STAGES_PASSED:-0}" \
            --argjson self_heal_count "${SELF_HEAL_COUNT:-0}" \
            --arg failed_stage "" \
            '{status: $status, issue_number: $issue, goal: $goal,
              task_type: $task_type, template: $template, duration_s: $duration_s,
              stages_passed: $stages_passed, self_heal_count: $self_heal_count,
              failed_stage: $failed_stage}' \
            > "$_outcome_file" 2>/dev/null || true
        if [[ -s "$_outcome_file" ]] && ruflo_available && declare -f ruflo_learn_from_shipwright >/dev/null 2>&1; then
...
stage_monitor() {
    ...
    while [[ "$poll" -lt "$total_polls" ]]; do
        ...
        if [[ "$total_errors" -ge "$error_threshold" ]]; then
            ...
            # Learning feedback: index failure outcome into ruflo HNSW
            if [[ -n "${ARTIFACTS_DIR:-}" ]]; then
                local _failure_outcome_file="$ARTIFACTS_DIR/pipeline-outcome-monitor-failure.json"
                local _mon_duration_s=0
                if [[ -n "${PIPELINE_START_EPOCH:-}" ]]; then
                    _mon_duration_s=$(( $(now_epoch) - PIPELINE_START_EPOCH ))
                fi
                jq -n \
                    --arg status "failure" \
                    --arg issue "${ISSUE_NUMBER:-}" \
                    ...
                    --arg task_type "${INTELLIGENCE_ISSUE_TYPE:-unknown}" \
                    --arg template "${PIPELINE_NAME:-standard}" \
                    --argjson duration_s "$_mon_duration_s" \
                    --argjson stages_passed "${PIPELINE_STAGES_PASSED:-0}" \
                    --argjson self_heal_count "${SELF_HEAL_COUNT:-0}" \
                    --arg failed_stage "monitor" \
                    --argjson total_errors "$total_errors" \
                    --argjson error_threshold "$error_threshold" \
                    '{status: $status, issue_number: $issue, goal: $goal,
                      task_type: $task_type, template: $template, duration_s: $duration_s,
                      stages_passed: $stages_passed, self_heal_count: $self_heal_count,
                      failed_stage: $failed_stage, total_errors: $total_errors,
                      error_threshold: $error_threshold}' \
                    > "$_failure_outcome_file" 2>/dev/null || true
                if [[ -s "$_failure_outcome_file" ]] && ruflo_available && declare -f ruflo_learn_from_shipwright >/dev/null 2>&1; then
                    ruflo_learn_from_shipwright "$_failure_outcome_file" || true
                fi
            fi
