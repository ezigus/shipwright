...
setup_env() {
    TEST_TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sw-launchd-test.XXXXXX")

    # Create necessary directories
    mkdir -p "$TEST_TEMP_DIR/home/.shipwright/logs"
    mkdir -p "$TEST_TEMP_DIR/home/Library/LaunchAgents"
    mkdir -p "$TEST_TEMP_DIR/systemd/user"
    mkdir -p "$TEST_TEMP_DIR/bin"
    mkdir -p "$TEST_TEMP_DIR/scripts"
    mkdir -p "$TEST_TEMP_DIR/project"

    # Copy the launchd script and helpers for color/output
    cp "$LAUNCHD_SCRIPT" "$TEST_TEMP_DIR/scripts/sw-launchd.sh"
...
create_mock_launchctl() {
    cat > "$TEST_TEMP_DIR/bin/launchctl" <<'EOF'
#!/usr/bin/env bash
MOCK_LAUNCHCTL_LOG="${TEST_TEMP_DIR}/launchctl-calls.log"
...
EOF
    chmod +x "$TEST_TEMP_DIR/bin/launchctl"
}
...
#!/usr/bin/env bash
MOCK_SYSTEMCTL_LOG="${TEST_TEMP_DIR}/systemctl-calls.log"
...
test_macos_detection() {
    ...
    local output
    output=$(OSTYPE="darwin19.6.0" bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" install 2>&1 || true)
...
test_linux_detection_systemd() {
    ...
    cat > "$TEST_TEMP_DIR/bin/systemctl" <<'MOCK'
#!/usr/bin/env bash
echo "Mock systemctl: $*" >> "$TEST_TEMP_DIR/systemctl.log"
MOCK
    chmod +x "$TEST_TEMP_DIR/bin/systemctl"

    # Run install on Linux — should succeed via systemd path
    local output exit_code=0
    output=$(OSTYPE="linux-gnu" bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" install 2>&1) || exit_code=$?
...
test_daemon_plist_structure() {
    ...
    OSTYPE="darwin19.6.0" bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" install 2>/dev/null || true
...
test_connect_plist_conditional() {
    ...
    HOME="$fresh_home" OSTYPE="darwin19.6.0" bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" install 2>/dev/null || true
...
test_install_calls_launchctl() {
    ...
    true > "$TEST_TEMP_DIR/launchctl-calls.log"
    ...
    local load_calls
    load_calls=$(grep -c "load" "$TEST_TEMP_DIR/launchctl-calls.log" || echo 0)
...
test_install_missing_sw_binary() {
    ...
    mkdir -p "$TEST_TEMP_DIR/bin-empty"

    # Run with minimal PATH so sw can't be found
    local output exit_code=0
    output=$(PATH="$TEST_TEMP_DIR/bin-empty:/usr/bin:/bin" OSTYPE="darwin19.6.0" SCRIPT_DIR="$TEST_TEMP_DIR/scripts/.." bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" install 2>&1) || exit_code=$?
...
test_uninstall_removes_plists() {
    ...
    # Now uninstall
    OSTYPE="darwin19.6.0" bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" uninstall 2>/dev/null || true
...
test_uninstall_calls_launchctl_unload() {
    ...
    local unload_calls
    unload_calls=$(grep -c "unload" "$TEST_TEMP_DIR/launchctl-calls.log" || echo 0)
...
test_uninstall_empty_system() {
    ...
    local exit_code=0
    OSTYPE="darwin19.6.0" bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" uninstall 2>/dev/null || exit_code=$?
...
test_status_checks_launchctl_list() {
    ...
    mkdir -p "$HOME/.shipwright"
    echo '{}' > "$HOME/.shipwright/team-config.json"
    rm -f "$TEST_TEMP_DIR/launchctl-calls.log"
    OSTYPE="darwin19.6.0" bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" install 2>/dev/null || true
    ...
    # Run status
    local output
    output=$(OSTYPE="darwin19.6.0" bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" status 2>&1)
...
test_help_command() {
    local output
    output=$(bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" help 2>&1)
...
test_unknown_command_shows_help() {
    local exit_code=0
    local output
    output=$(bash "$TEST_TEMP_DIR/scripts/sw-launchd.sh" invalid-command 2>&1) || exit_code=$?
...
test_version_defined() {
    if ! grep -q "^VERSION=" "$TEST_TEMP_DIR/scripts/sw-launchd.sh"; then
        echo -e "    ${RED}✗${RESET} VERSION variable not defined"
        return 1
    fi

    local version
    version=$(grep "^VERSION=" "$TEST_TEMP_DIR/scripts/sw-launchd.sh" | cut -d'=' -f2)
