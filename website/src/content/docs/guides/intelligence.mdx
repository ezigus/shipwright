...
Intelligence is **enabled by default** with optimization and prediction active out of the box. Override in `.claude/daemon-config.json`:
...
**Default configuration** (ships in `.claude/daemon-config.json`):

- `enabled: true` — Master switch (on by default)
- `optimization_enabled: true` — Learn from every pipeline run (on by default)
- `prediction_enabled: true` — Risk assessment before pipeline starts (on by default)
...
The self-optimization module includes a context efficiency feedback loop that analyzes how well pipeline agents use their context budgets:

- **Budget utilization tracking** — Measures what percentage of the context budget each iteration uses
- **Trim ratio monitoring** — Tracks how much content gets trimmed by the context engineering layer
- **Automatic recommendations** — When utilization is consistently high (>90%), recommends increasing budget; when low (<50%), recommends decreasing to save tokens; when trim ratio is high (>30%), recommends reducing verbose context sections

```bash
shipwright self-optimize tune  # Includes context efficiency analysis
```

Context efficiency events are also visible in the dashboard via the `/api/context-efficiency` endpoint.
...
| `optimization_enabled` | `true`  | Enable self-tuning based on DORA metrics             |
| `prediction_enabled`   | `true`  | Enable risk assessment and anomaly detection         |
...
| `ab_test_ratio`        | `0.2`   | Fraction of runs using composed pipelines (A/B test) |
...
Optimization and prediction are enabled by default. Enable additional features in order of impact:

1. **Default:** `optimization_enabled: true` + `prediction_enabled: true` — Active out of the box
2. **Phase 2:** `simulation_enabled: true` — Simulated PR review (adds time)
3. **Phase 3:** `adversarial_enabled: true` — Red-team review (slower)
4. **Advanced:** `architecture_enabled: true`, `composer_enabled: true`

The default configuration gives immediate feedback without slowing pipelines significantly.
