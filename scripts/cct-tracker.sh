...
load_tracker_config() {
    ...
    case "$TRACKER_PROVIDER" in
        linear)
            if [[ -f "$SCRIPT_DIR/cct-tracker-linear.sh" ]]; then
                source "$SCRIPT_DIR/cct-tracker-linear.sh"
                return 0
            else
                warn "Linear provider script not found: $SCRIPT_DIR/cct-tracker-linear.sh"
            fi
            ;;
        jira)
            if [[ -f "$SCRIPT_DIR/cct-tracker-jira.sh" ]]; then
                source "$SCRIPT_DIR/cct-tracker-jira.sh"
                return 0
            else
                warn "Jira provider script not found: $SCRIPT_DIR/cct-tracker-jira.sh"
            fi
            ;;
        none|"") return 0 ;;
        *)
...
show_help() {
    ...
    echo -e "  Config file:  ${DIM}~/.claude-teams/tracker-config.json${RESET}"
