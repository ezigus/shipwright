...
# ──────────────────────────────────────────────────────────────────────────────
test_add_machine() {
    rm -f "$TEMP_DIR/home/.claude-teams/machines.json"

    HOME="$TEMP_DIR/home" \
    PATH="$TEMP_DIR/bin:$PATH" \
        bash "$TEMP_DIR/scripts/cct-remote.sh" add "builder-1" \
            --host "localhost" \
...
test_add_second_machine() {
    ...
    HOME="$TEMP_DIR/home" \
    PATH="$TEMP_DIR/bin:$PATH" \
        bash "$TEMP_DIR/scripts/cct-remote.sh" add "builder-2" \
            --host "localhost" \
            --path "$TEMP_DIR/mock-install" \
...
test_remove_machine() {
    HOME="$TEMP_DIR/home" \
    PATH="$TEMP_DIR/bin:$PATH" \
        bash "$TEMP_DIR/scripts/cct-remote.sh" remove "builder-2" 2>/dev/null
...
test_atomic_write() {
    ...
    local script="$TEMP_DIR/scripts/cct-remote.sh"
...
test_status_heartbeat_section() {
    local status="$SCRIPT_DIR/cct-status.sh"
