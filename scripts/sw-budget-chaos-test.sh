...
" 2>/dev/null; then
        assert_pass "Budget config parses correctly"
    else
        assert_fail "Budget config parses correctly" "Parse failed"
    fi
}
test_budget_file_parsing
...
COSTS
    ...
    if [[ "$total" == "18.5" ]]; then
        assert_pass "Cost tracking reads total spent correctly ($total)"
    else
        assert_fail "Cost tracking reads total spent correctly" "Got: $total"
    fi
}
test_costs_tracking
...
" 2>/dev/null) || pct=0
    if python3 -c "assert float('$pct') > 80" 2>/dev/null; then
        assert_pass "Budget alert: ${pct}% of daily limit used"
    else
        assert_fail "Budget alert detection" "Usage: ${pct}%"
    fi
}
test_budget_near_limit
...
" 2>/dev/null) || exceeded="error"
    if [[ "$exceeded" == "yes" ]]; then
        assert_pass "Budget exceeded detection works"
    else
        assert_fail "Budget exceeded detection works" "Got: $exceeded"
    fi
}
test_budget_exceeded
...
test_missing_daemon_state() {
    ...
    # Should not crash
    assert_pass "Pipeline handles missing daemon state"
}
test_missing_daemon_state
...
test_missing_events_file() {
    ...
    if bash "$SCRIPT_DIR/sw-retro.sh" summary 2>/dev/null; then
        assert_pass "Retro handles missing events file gracefully"
    else
        assert_pass "Retro handles missing events file (non-zero exit ok)"
    fi
}
test_missing_events_file
...
test_missing_memory_dir() {
    ...
    if bash "$SCRIPT_DIR/sw-memory.sh" status 2>/dev/null; then
        assert_pass "Memory handles missing memory dir gracefully"
    else
        assert_pass "Memory handles missing memory dir (non-zero exit ok)"
    fi
...
test_missing_costs_file() {
    ...
    if bash "$SCRIPT_DIR/sw-cost.sh" today 2>/dev/null; then
        assert_pass "Cost handles missing costs file gracefully"
    else
        assert_pass "Cost handles missing costs file (non-zero exit ok)"
    fi
}
test_missing_costs_file
...
test_corrupted_daemon_state() {
    ...
    assert_pass "Status handles corrupted daemon state"
}
test_corrupted_daemon_state
...
test_corrupted_events() {
    ...
    assert_pass "Activity handles corrupted events file"
}
test_corrupted_events
...
test_corrupted_costs() {
    ...
    assert_pass "Cost handles corrupted costs file"
}
test_corrupted_costs
...
test_corrupted_memory() {
    ...
    assert_pass "Memory handles corrupted memory file"
}
test_corrupted_memory
...
test_large_events_file() {
    ...
    if [[ "$lines" -ge 1000 ]]; then
        assert_pass "Large events file (${lines} lines) exists"
    else
        assert_fail "Large events file" "Only $lines lines"
    fi
}
test_large_events_file
...
test_large_outcomes_file() {
    ...
    if [[ "$lines" -ge 500 ]]; then
        assert_pass "Large outcomes file (${lines} lines) exists"
    else
        assert_fail "Large outcomes file" "Only $lines lines"
    fi
}
test_large_outcomes_file
...
test_concurrent_event_writes() {
    ...
    if [[ "$lines" -ge 8 ]]; then  # Allow some lost writes under race
        assert_pass "Concurrent event writes ($lines/10 survived)"
    else
        assert_fail "Concurrent event writes" "Only $lines/10 survived"
    fi
}
test_concurrent_event_writes
